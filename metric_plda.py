...
def calculate_FAR_FRR(positive_scores, negative_scores, threshold=-10):
    # Calculate False Rejection Rate (FRR)
    pos_scores_threshold = positive_scores <= threshold
    FRR = (pos_scores_threshold.sum(0)).float() / positive_scores.shape[0]

    # Calculate False Acceptance Rate (FAR)
    neg_scores_threshold = negative_scores > threshold
    FAR = (neg_scores_threshold.sum(0)).float() / negative_scores.shape[0]

    # Calculate accuracy
    correct_positives = positive_scores.shape[0] * (1 - FRR)
    correct_negatives = negative_scores.shape[0] * (1 - FAR)
    total_samples = positive_scores.shape[0] + negative_scores.shape[0]
    accuracy = (correct_positives + correct_negatives) / total_samples

    return float(FAR), float(FRR), float(accuracy)
...
def EER(positive_scores, negative_scores):
    ...
    final_FRR = 0
    final_FAR = 0
    # List to store FAR and FRR for each threshold
    for i, cur_thresh in enumerate(thresholds):
        ...
        # Save FAR and FRR for the current threshold
        #far_frr_list.append((float(cur_thresh), float(FAR), float(FRR)))
