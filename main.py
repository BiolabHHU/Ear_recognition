...
from metric_plda import calculate_FAR_FRR
...
class XVectorModel(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        self.speaker_encoder = MainModel(nOut=256)
    ...
    def train_dataloader(self):
        ...
        train_data_loader = DataLoader(dataset=self.dataset, batch_size=self.batch_size, num_workers=0, shuffle=True)
        return train_data_loader

    # Load only the validation data
    def val_dataloader(self):
        self.dataset.load_data(val=True)
        val_data_loader = DataLoader(dataset=self.dataset, batch_size=self.batch_size, num_workers=0, shuffle=False)
    ...
    def test_dataloader(self):
        if extract_mode == 'train':
            ...
            test_data_loader = DataLoader(dataset=self.dataset, batch_size=self.batch_size, num_workers=0,
                                          shuffle=False)
...
if __name__ == "__main__":
    ...
    config = Config(data_folder_path='/home/hhdx/PycharmProjects/yyds/yds_paper/ear_recognition',
                    # checkpoint_path='D:/pycharm/PyCharm Community Edition 2021.3/jbr/bin/testlogs/lightning_logs/version_97/checkpoints/epoch=38-step=507.ckpt',
    ...
    if config.extract_x_vectors:
        ...
        if config.train_x_vector_model:
            ...
            x_vector.to_csv('x_vectors/x_vector_train_v1_5_l7.csv')  # TODO set to default name
        ...
        if (config.train_x_vector_model):
            trainer.test(model)
            x_vector = pd.DataFrame(x_vector)
            x_vector.to_csv('x_vectors/x_vector_test1_v1_5_l7.csv')  # TODO set to default name
        elif (config.checkpoint_path != 'none'):
            trainer.test(model, ckpt_path=config.checkpoint_path)
            x_vector = pd.DataFrame(x_vector)
            x_vector.to_csv('x_vectors/x_vector_test1_v1_5_l7.csv')  # TODO set to default name
    ...
    for i in range(1, 11):
        ...
        if config.train_plda:
            ...
            x_vectors_data = pd.read_csv('x_vectors/x_vector_test1_v1_5_l7.csv').iloc[0:900]
        ...
        if (config.test_plda):
            ...
            plda = pc.load_plda('plda/plda_ivec_v2_d50.pickle')  # TODO set to default name
            # x_vectors_test = pd.read_csv('x_vectors/x_vector_test1_v1_5_l7.csv').iloc[0:300]
            ...
            x_vectors_train = pd.read_csv('x_vectors/x_vector_test1_v1_5_l7.csv').iloc[0:900]
            x_id_train = np.array(x_vectors_train.iloc[:, 1])
