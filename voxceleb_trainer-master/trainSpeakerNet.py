...
def main_worker(cpu, ncpus_per_node, args):
    ...
    if args.distributed:
        ...
        dist.init_process_group(backend='nccl', world_size=ncpus_per_node, rank=args.cpu)

        torch.cuda.set_device(args.cpu)
        s.cuda(args.cpu)

        s = torch.nn.parallel.DistributedDataParallel(s, device_ids=[args.cpu], find_unused_parameters=True)

        print('Loaded the model on GPU {:d}'.format(args.cpu))

    else:
        s = WrappedModel(s).cuda(args.cpu)
