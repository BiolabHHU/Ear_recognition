...
class TdnnLayer0(nn.Module):
    def __init__(self, input_size=24, output_size=512, context=[0], batch_norm=True, dropout_p=0.0):
        ...
        """
        super(TdnnLayer0, self).__init__()
        ...
        # 创建3个全连接层
        self.linear1 = nn.Linear(13, 40)
        self.linear2 = nn.Linear(13, 40)
        self.linear3 = nn.Linear(13, 48)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2, x3), dim=2)  # 合并3个全连接层的结果
        x = self.relu(x)
        ...
        return x
class TdnnLayer00(nn.Module):
    def __init__(self, input_size=24, output_size=512, context=[0], batch_norm=True, dropout_p=0.0):
        """
        TDNN as defined by https://www.danielpovey.com/files/2015_interspeech_multisplice.pdf
        Structure inspired by https://github.com/cvqluu/TDNN/blob/master/tdnn.py
        """
        super(TdnnLayer00, self).__init__()
        ...
        # 创建3个全连接层
        self.linear1 = nn.Linear(64, 20)
        self.linear2 = nn.Linear(64, 20)
        self.linear3 = nn.Linear(64, 24)
...
class TdnnLayer1(nn.Module):
    def __init__(self, input_size=13, output_size=512, context=[0], batch_norm=True, dropout_p=0.0):
        ...
        # 创建5个全连接层
        self.linear1 = nn.Linear(13, 24)
        self.linear2 = nn.Linear(13, 24)
        self.linear3 = nn.Linear(13, 24)
        self.linear4 = nn.Linear(13, 24)
        self.linear5 = nn.Linear(13, 32)
